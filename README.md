...
1. `swift build`
1. `.build/debug/SimpleImageRecognitionApp`
...
* `docker run -it -p 8080:8080 -v $PWD:/swift-project -w /swift-project myapp-run sh -c .build-ubuntu/release/SimpleImageRecognitionApp`


#### Kubernetes
To deploy your application to your Kubernetes cluster, run `helm install --name myapp .` in the `/chart/SimpleImageRecognitionApp` directory. You need to make sure you change the `repository` variable in your `chart/SimpleImageRecognitionApp/values.yaml` file points to the docker image containing your runnable application.
...
### Generator
This project was generated with [generator-swiftserver](https://github.com/IBM-Swift/generator-swiftserver) v5.12.0.
# Kitura Image Recognition Example app
