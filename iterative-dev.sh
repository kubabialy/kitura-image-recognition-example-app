...
function killServerProcesses() {
    pid="$(pgrep SimpleImageRecognitionApp)"
    if [ "$pid" != "" ]
    then
        echo "killing SimpleImageRecognitionApp"
        kill $pid
    fi
